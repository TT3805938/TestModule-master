    ...
    t.extend(i.prototype, {
        init: function () {
            ...
            a.settings.init && a.settings.init(), a.cart_empty(), a.product.each(function (e) {
                var n = t(this),id=n.find("."+a.settings.product_id), i = n.find("." + a.settings.product_title), r = n.find("." + a.settings.product_price), s = n.find("." + a.settings.product_button), c = i.text(), o = r.text().replace(",", "").replace(/[^\d.]/g, "");
                s.on("click touchend", function (t) {
                    ...
                    var s, u = n.attr("data-id", c.replace(/ /g, "") + r.text().replace(/\r\n|\r|\n/g, "").replace(/[^a-zA-Z 0-9]+/g, "") + "_" + id.text()).attr("data-id"), l = a.cart.find("li"), _ = a.cart.find("[data-id=" + u + "]"), p = _.attr("data-id"), d = l.find("." + a.settings.cart + "-input");
                    //alert(u);
                    //alert("??");
                    d.val();
            ...
            //alert("进入add_to_cart方法");
            var array=a.split('_');
            var id=array[1];
            var n = this;
            //alert(id);
           // alert("跑到这里了!");
            n.cart.append("true" === n.settings.currency_after_number ? '<li class="animated ' + n.settings.animation + '" data-id=' + id + "><span class=" + n.settings.cart + "-id>" + id + "</span><span class=" + n.settings.cart + "-name>" + t + "</span><span class=" + n.settings.cart + "-price>" + e + n.settings.currency + '</span><input type="number" min="1" value="1" class=' + n.settings.cart + "-input><button class=" + n.settings.cart + "-remove>&times;</button></li>" : '<li class="animated ' + n.settings.animation + '" data-id=' + a + "><span class=" + n.settings.cart + "-name>" + t + "</span><span class=" + n.settings.cart + "-price>" + n.settings.currency + e + '</span><input type="number" min="1" value="1" class=' + n.settings.cart + "-input><button class=" + n.settings.cart + "-remove>&times;</button></li>"),n.storage_set()
        }, add_cart_buttons: function () {
            var t = this;
            t.cart.parent().append('<button class="' + t.settings.cart + '-clear">清空所有</button>'), t.cart.parent().append('<button type="submit" class="' + t.settings.cart + '-checkout">提交订单</button>'), t.clear_cart(), t.checkout_button()
        }, checkout: function () {
            ...
            //t.extend(n, a.settings.paypal), i.attr("action", ""), i.attr("method", "post"), i.attr("target", "_blank");
            var strList="";
            for (var s in n)e = t('<input type="hidden" />'), e.attr("name", s), e.attr("value", n[s]), i.append(e);
            a.cart.find("li").each(function (e) {
                var n = t(this);//alert(n.attr("data-id"));
                   var id=n.attr("data-id"), s = n.find("." + a.settings.cart + "-name").text(), c = parseFloat(n.find("." + a.settings.cart + "-price").text().replace(/^\D+/g, "")), o = n.find("input").val(), u = {
                    name: t("<input />", {
                        type: "hidden",
                        name: r.name + "_" + (e + 1),
                        value: s
                ...
                //alert("ID:"+id.split('_')[1]+"名字:"+s+"数量:"+o+"价格:"+c);//数量
                //上传到服务器
                strList+=id.split('_')[1]+","+s+","+o+","+(Number(c)*Number(o)).toFixed(2)+"|";
                //alert(strList);
            ...
            //alert(GetQueryString("Name"));
                $.ajax({
                    method: "Post",
                    url: "http://192.168.1.106:8080/api/DingCan/AddOrder",
                    data: {Name:GetQueryString("Name"),Phone:GetQueryString("Phone"),Address:GetQueryString("Address"),ShopID:1,List:strList,OrderTime:GetQueryString("OrderTime"),OrderDate:GetQueryString("OrderDate")}
                }).done(function (ndata) {
                    ...
                    if (ddd.Code == 200) {
                      ...
                      //       window.open('Order.html', '_self', '');
                        window.location.href="Order.html?List="+strList;
                    }
                    else {
                        alert(ddd.Msg);
                    }
            ...
            $(t.cart).find("li").each(function () {
                ...
                //alert(n);
                  var  i = e.find("." + t.settings.cart + "-name").text(), r = e.find("." + t.settings.cart + "-price").text(), s = e.find("input").val(), c = {
                    name: i,
                    id: n,
                    price: r,
                    input: s
